...
<body>
    ...
    <script>
        ...
        function initFallbackMode() {
            ...
            container.innerHTML = \`
                <div style="position: relative; width: 100%; height: 100%; background: #f0f0f0;">
                    <div id="fallbackCanvas" style="width: 100%; height: 100%; position: relative; overflow: hidden;">
                        <h3 style="text-align: center; margin-top: 50px;">Interactive Neural Network (Fallback Mode)</h3>
                        <p style="text-align: center;">Click buttons below to create draggable neurons!</p>
                        <div id="neuronContainer" style="position: relative; width: 100%; height: 300px;"></div>
                    </div>
                </div>
            \`;
            updateStatus('Fallback mode ready - neurons will be draggable!');
        }
        ...
        function createDraggableNeuron(x, y, label, type) {
            ...
            neuron.draggable = true;
            
            // Add drag functionality
            let isDragging = false;
            let startX, startY, initialX, initialY;
            
            neuron.onmousedown = function(e) {
                isDragging = true;
                startX = e.clientX;
                startY = e.clientY;
            ...
            document.onmousemove = function(e) {
                if (isDragging) {
                    ...
                    neuron.style.left = (initialX + deltaX) + 'px';
                    neuron.style.top = (initialY + deltaY) + 'px';
                }
            };
            ...
            document.onmouseup = function() {
                if (isDragging) {
                    ...
                    updateStatus(\`Moved \${label} to position (\${neuron.offsetLeft}, \${neuron.offsetTop})\`);
                }
            };
        ...
        function createSampleNetwork() {
            ...
            if (graph) {
                ...
                // Fallback mode
                for (let i = 0; i < 4; i++) {
                    createNeuron(50, 20 + i * 60, \`I\${i+1}\`, 'inputNeuron');
                }
                for (let i = 0; i < 6; i++) {
                    createNeuron(200, 10 + i * 45, \`H\${i+1}\`, 'hiddenNeuron');
                }
                for (let i = 0; i < 2; i++) {
                    createNeuron(350, 50 + i * 60, \`O\${i+1}\`, 'outputNeuron');
                }
            }
