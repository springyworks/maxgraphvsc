...
<body>
    ...
    <script>
        ...
        function createDraggableNeuron(x, y, label, type) {
            ...
            neuron.draggable = false; // Disable HTML5 drag, use mouse events instead
        ...
        function createSampleNetwork() {
            ...
            if (graph) {
                ...
                // Fallback mode - create fully draggable sample network
                console.log('Creating fallback sample network with draggable neurons');
                ...
                // Create input layer (draggable)
                for (let i = 0; i < 4; i++) {
                    createNeuron(50, 40 + i * 70, \`I\${i+1}\`, 'inputNeuron');
                }
                
                // Create hidden layer (draggable)
                for (let i = 0; i < 6; i++) {
                    createNeuron(200, 20 + i * 50, \`H\${i+1}\`, 'hiddenNeuron');
                }
                
                // Create output layer (draggable)
                for (let i = 0; i < 2; i++) {
                    createNeuron(350, 80 + i * 70, \`O\${i+1}\`, 'outputNeuron');
                }
                
                console.log('Fallback sample network created with', nodeCounter, 'draggable neurons');
            }
        ...
        function clearNetwork() {
            if (graph && typeof mxGraph !== 'undefined') {
                ...
                // Fallback mode - remove all draggable neurons
                const container = document.getElementById('neuronContainer') || document.getElementById('graphContainer');
                if (container) {
